...
async function payWithPaystack(){
  ...
  paystack.newTransaction({
    ...
    onSuccess: async function(response){
      ...
      let fee = amount * 0.02;
      let netCredit = amount - fee;
      let newBalance = oldBalance + netCredit; 
      ...
      if(window.sendSmsNotification && userData.phone) {
        window.sendSmsNotification(userData.phone, `Wallet Funded: ₵${netCredit.toFixed(2)} has been successfully credited to your Data4Ghana account.`);
      }

      if(window.showSuccessPopup) {
        window.showSuccessPopup("Wallet Funded!", "Your wallet has been successfully credited with ₵" + netCredit.toFixed(2) + ".", () => {
          window.location.reload();
        });
      } else {
        alert("Payment successful! Wallet credited with ₵" + netCredit.toFixed(2));
        window.location.reload();
      }
    },
    onCancel: function(){
