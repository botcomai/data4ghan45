...
async function loadAfaPricing() {
  try {
    ...
    const { data: prices } = await supabase.from('pricing').select('price').eq('role', userRole).eq('product', 'afa_normal').single();

    if (prices) {
        afaNormalPrice = parseFloat(prices.price);
        document.querySelector('.normal-price-label').textContent = `₵${afaNormalPrice.toFixed(2)}`;
    }
  } catch (e) {
...
async function loadNormalHistory() {
  try {
    ...
    if (!history || history.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#94a3b8;">No normal registrations found.</td></tr>';
      return;
    }
    ...
    document.querySelector('#normalHistoryTable tbody').innerHTML = '<tr><td colspan="5" style="text-align:center; color:#ef4444;">Failed to load history.</td></tr>';
  }
}
