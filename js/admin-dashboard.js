...
async function loadOverviewStats() {
    try {
        ...
        // 1. Total (Real) Users - Excluding admins/super_agents
        const { count: realUserCount } = await supabase
            .from('users')
            .select('*', { count: 'exact', head: true })
            .not('role', 'in', '(admin,super_agent)');
        document.getElementById("metric-real-users").innerText = realUserCount || 0;
        ...
        try {
            ...
            const { count: pendingCount } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'false');
            ...
            document.getElementById("overview-count-pending").innerText = pendingCount || 0;
            document.getElementById("overview-count-completed").innerText = completedCount || 0;
            document.getElementById("overview-count-processing").innerText = processingCount || 0;
...
async function fetchComprehensiveTotals() {
    try {
        ...
        const { data: orders } = await supabase.from('orders').select('network, plan, bundle, amount').eq('status', 'true');
        ...
        if (orders) {
            ...
            orders.forEach(order => {
                ...
                const baseCost = getBaseCost(network, volumeNum);
...
function updateOrderStatusCounts() {
    ...
    allOrders.forEach(o => {
        ...
        if (s === 'false') counts.pending++;
        else if (s === 'true') counts.completed++;
...
async function loadPricingConfig() {
    try {
        ...
        const { data: settings } = await supabase
            .from('app_settings')
            .select('key, value')
            .in('key', ['paystack_public_key', 'paystack_secret_key', 'paystack_enabled', 'manual_transfer_enabled', 'manual_momo_number', 'manual_momo_name']);
...
async function loadSupportTickets() {
    ...
    data.forEach(t => {
        ...
        html += `
            <tr>
                <td style="font-family:monospace; color:#64748b;">${t.id.substring(0,8)}</td>
                <td style="font-family:monospace; color:#2563eb;">${t.order_id ? t.order_id.substring(0,8) : 'General'}</td>
...
async function exportAllUsersCSV() {
    ...
    try {
        ...
        while (more) {
            const { data, error } = await supabase
                .from('users')
                .select('first_name, last_name, email, phone, wallet_balance, role, created_at')
        ...
        const headers = ["First Name", "Last Name", "Email", "Phone", "Wallet Balance", "Role", "Joined Date"];
...
async function generateProfitReport() {
    ...
    try {
        let query = supabase.from('orders').select('*, users!inner(role)').eq('status', 'true');
...
function getStatusBadge(status) {
    ...
    } else if (s === 'false' || s === 'pending' || s === 'scheduled') {
        bg = '#fee2e2'; text = '#dc2626'; dot = '#ef4444'; label = s.toUpperCase();
