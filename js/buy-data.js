  ...
  try {
    ...
    if (!isBulk) {
        ...
        if (bundleSelect) {
            ...
            // Standard sizes to show if not in DB
            const standardSizes = [1, 2, 5, 10];
            const allSizes = [...new Set([...standardSizes, ...variants.map(v => v.gb_size).filter(s => s > 0)])].sort((a,b) => a-b);
  ...
  try {
    ...
    bundleSelect.addEventListener('change', () => {
      ...
      if (val && preview && previewText) {
        ...
        previewText.innerHTML = `${val}GB ${selectedNetwork} data will cost <strong>₵${exactPrice.toFixed(2)}</strong> at your <em>${(roleConfig[currentUserRoleForPricing] || roleConfig['client']).label}</em> rate`;
        preview.style.display = 'block';
      } else if (preview) {
        preview.style.display = 'none';
      }
